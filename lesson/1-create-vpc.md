...
sed -i "" "s|\${MY_IP}|$MY_IP|g" $(pwd)/cf/inf-vpc.yaml
```
vpc 를 생성한다.
```
aws cloudformation create-stack \
  --region ${AWS_REGION} \
  --stack-name infer-on-eks \
  --template-body file://$(pwd)/cf/inf-vpc.yaml \
  --parameters ParameterKey=KeyPairName,ParameterValue=${KEYPAIR_NAME} \
  --capabilities CAPABILITY_NAMED_IAM \
  --tags Key=Project,Value=infer-on-eks
```
vpc 생성 진행 과정을 조회하고 완료될때 까지 대기한다. 
```
aws cloudformation describe-stacks --stack-name infer-on-eks --query "Stacks[0].StackStatus"
```
